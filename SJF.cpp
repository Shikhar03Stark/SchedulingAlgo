...
shAlgo::SJF::SJF():processQ(basis::BURST){
    isPreemptive = false;
    iterator = 0;
    currentTimePoint = 0;
    startTimePoint = 0;
...
shAlgo::SJF::SJF(int totProcess, bool nature):processQ(basis::BURST){
    isPreemptive = nature;
    iterator = 0;
    currentTimePoint = 0;
    startTimePoint = 0;
...
void shAlgo::SJF::insertProcess(Process* p){
    ...
    if(isPreemptive){
        while(currentTimePoint <= stopTimePoint){
            ...
            if(runningProcess && processQ.getSize() != 0){
                ...
                if(p->getExpectedBT() < runningProcess->getExpectedBT()){
                    //switch conetext
